...
export function Navbar() {
	...
	const menuItems = [
		{ path: "/", name: "Home", icon: <Dices className='w-4 h-4 mr-2' /> },
		{
			path: "/leaderboard",
			name: "Leaderboard",
	...
	return (
		<nav className='sticky top-0 z-50 border-b border-[#EA8105]/40 backdrop-blur-md bg-[#191F3B]/95 text-white shadow-md'>
				...
				<div className='flex items-center gap-3'>
					<Link to='/' className='flex items-center gap-2'>
						<img
							...
							alt='pnpplxprss Logo'
							className='object-cover w-8 h-8 md:w-10 md:h-10 border rounded-full shadow-sm border-[#EA8105]/60'
						/>
						{!isMobile && (
							<span className='text-xl md:text-2xl font-bold text-[#C33B52] select-none'>
								PnpplXprss
							</span>
						)}
					</Link>
					...
					{isLive ? (
						<span className='px-2 py-0.5 text-xs bg-[#AF2D03] text-white rounded-full font-semibold animate-pulse select-none'>
							🔴 LIVE {viewerCount !== null ? `(${viewerCount})` : ""}
						</span>
					) : (
						<span className='px-2 py-0.5 text-xs bg-[#C33B52] text-white rounded-full font-semibold select-none'>
							Offline
						</span>
					)}
				</div>
			...
			{/* Mobile Menu */}
			{isMobile && isOpen && (
				<div className='fixed inset-0 z-40 bg-[#191F3B]/95 backdrop-blur-md pt-16'>
								...
								{user ? (
									<>
										...
										<Button
											variant='outline'
											size='lg'
											className='w-full justify-start border-white text-white hover:bg-[#AF2D03] hover:border-[#AF2D03]'
											onClick={() => {
												logout();
												setIsOpen(false);
											}}
									...
									<>
										<Button
											variant='outline'
											size='lg'
											className='w-full justify-start border-white text-[#ffffff] hover:bg-[#EA8105] hover:border-[#EA8105]'
											asChild
										>
											<Link
												to='/login'
										...
										<Button
											size='lg'
											className='w-full justify-start text-[#ffffff] hover:text-[#EA8105] font-semibold'
											asChild
										>
